...
% 中英文字型（Fonts）
%
% 填 fonts/english/、fonts/chinese/ 裡的字型檔（含副檔名），或系統已安裝的字族
% 名稱，找不到就報錯。options 原樣交給 fontspec：字族名稱會自動配粗體與斜體，
% 可留空；檔名則要自己指定。規範用 Times New Roman，有安裝就改填。
%
% A font file in fonts/english/ or fonts/chinese/, extension included, or an
% installed font family; neither found is an error. The options go to fontspec:
% a family name pairs its own bold and italic, a file needs them named. The NTU
% rules name Times New Roman — write {Times New Roman} if you have it.
% --------------------------------------------------
\ntufontsetup{
