...
\documentclass[
  ...
  fontset      = default,           % fontset = default | tinos | template | system | overleaf
                                    %   default  系統的 Times New Roman ＋ 隨附全字庫字型
                                    %   tinos    隨附的 Tinos ＋ 全字庫，完全不需安裝字型
                                    %   template 使用者自行放進 fonts/ 的字型（見 fonts/README.md）
                                    %   system   中英文都由作業系統解析字族名稱
                                    %   overleaf Overleaf 影像內建字型
  cjkfont      = kai,               % cjkfont = kai | sung
                                    %   kai  全字庫正楷體 TW-Kai（規範以楷書為主）
                                    %   sung 全字庫正宋體 TW-Sung
