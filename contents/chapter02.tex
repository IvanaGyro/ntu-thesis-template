...
% 這四張圖由 scripts/make_example_figures.py 產生，PDF 已經納入版本控制，
% 所以 Overleaf 上不需要執行任何 Python。
% The four PDFs are committed, so Overleaf never runs Python. Regenerate them
% with scripts/make_example_figures.py.
...
Citations are managed by \texttt{biblatex} and \texttt{biber}. The style
configured in \texttt{ntusetup.tex} is \texttt{numeric-comp}, which compresses
consecutive numbers into a range.

A plain citation looks like this~\cite{ex:article}. Several at once collapse
automatically~\cite{ex:book,ex:inproceedings,ex:thesis}. \texttt{biblatex} also
offers \verb|\parencite| for a parenthesised form~\parencite{ex:report} and
\verb|\textcite| for one that reads as part of the
sentence: \textcite{ex:software} makes this point directly.

The bibliography in \texttt{back/references.bib} carries one worked example of
every entry type \texttt{biblatex} supports, so you can copy the closest match
rather than guessing which fields a type requires.
